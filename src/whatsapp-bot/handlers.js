  ...
  try {
    ...
    if (interviewStates.includes(currentSession.state)) {
      await bot.sendMessage(
        from,
        "⚠️ *¡Espera un momento!* Ya tienes una entrevista en curso. Para reiniciar, envía *!reset* primero. ¡Ánimo con tu entrevista actual! 🚀"
      );
      logger.info(
        `Start command ignored for user ${from} due to active interview session`
      );
      return;
    }
    ...
    await bot.sendMessage(
      from,
      "😓 Lo siento, ha ocurrido un error al iniciar. Por favor, intenta nuevamente enviando *!start*."
    );
  }
};
...
const handleMenuSelection = async (from, selection) => {
  try {
    ...
    switch (selection) {
      case "review_cv":
        ...
        if (shouldPay) {
          ...
          const remainingCredits = await userService.getRemainingCVCredits(
            from
          );
          ...
          await sessionService.updateSessionState(
            from,
            "waiting_for_position_before_cv"
          );
      ...
      case "interview_simulation":
        ...
        await sessionService.updateSessionState(
          from,
          "waiting_for_position_before_interview"
        );
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al procesar tu selección. Por favor, intenta nuevamente con !start."
    );
  }
};
...
const handleDocument = async (from, document) => {
  try {
    ...
    if (shouldPay) {
      logger.info(
        `User ${from} has already analyzed a CV and needs to pay for more`
      );
      await handlePremiumInfo(from);
      return;
    }
    ...
    logger.info(
      `Document object received: ${JSON.stringify(document, null, 2)}`
    );
    ...
    try {
      ...
      if (typeof analysis === "string") {
        ...
        logger.info(
          `No URL found in analysis response, using fallback URL: ${analysisUrl}`
        );
      }
      ...
      await sessionService.updateSession(from, {
        previousAnalysis: session.previousAnalysis
          ? [...session.previousAnalysis, analysisUrl]
          : [analysisUrl],
      ...
      await userService.recordCVAnalysis(
        from,
        { url: analysisUrl },
        jobPosition
      );
      ...
      try {
        ...
        await bot.sendMessage(
          from,
          "Haz clic en el enlace anterior para ver tu análisis completo en PDF"
        );
        ...
        try {
          ...
          logger.error(
            `Error sending alternate link: ${simpleMessageError.message}`
          );
        }
      }

      logger.info(
        `CV analysis process completed for user ${from}. PDF URL: ${analysisUrl}`
      );
      return analysisUrl;
    } catch (error) {
    ...
    await bot.sendMessage(
      from,
      `⚠️ Lo siento, ocurrió un error al procesar tu CV: ${error.message}. Por favor, intenta nuevamente más tarde.`
    );
  }
};
...
const handleText = async (from, text) => {
  try {
    ...
    logger.info(
      `Handling text message from user ${from}. Session state: ${session.state}, hasReceivedWelcomeMessage: ${session.hasReceivedWelcomeMessage}`
    );

    if (
      /¡*hola,*\s*worky!*\s*soy\s*estudiante\s*de\s*(la\s*)*ucal/i.test(
        text.trim()
      )
    ) {
      const code = "UCAL20";
      ...
      try {
        ...
        if (userDoc.hasUnlimitedAccess) {
          await bot.sendMessage(
            from,
            "✨ ¡Ya tienes acceso ilimitado activado como estudiante UCAL!"
          );
          return;
        }
        ...
        if (userDoc.redeemedPromoCode) {
          await bot.sendMessage(
            from,
            `⚠️ Ya has canjeado un código promocional (${userDoc.redeemedPromoCode}). Solo se permite un código por usuario.`
          );
          return;
        }
        ...
        if (!codeData) {
          ...
          await bot.sendMessage(
            from,
            "❌ Error al activar código UCAL. Por favor contacta a soporte. "
          );
          return;
        }
        ...
        if (redeemed) {
          ...
          await bot.sendMessage(
            from,
            '⚠️ Hubo un problema al activar tu código UCAL. Por favor, contacta a soporte mencionando "error activación UCAL20".'
          );
          return;
        }
      } catch (error) {
        ...
        await bot.sendMessage(
          from,
          "⚠️ Ocurrió un error inesperado. Por favor, intenta nuevamente o contacta a soporte."
        );
        return;
      }
    }
    ...
    if (
      session.state === sessionService.SessionState.INITIAL &&
      !session.hasReceivedWelcomeMessage
    ) {
    ...
    if (text.toLowerCase().startsWith("!")) {
      ...
      switch (command) {
        ...
        case "clearsession":
          ...
          await bot.sendMessage(
            from,
            "✅ Tu sesión ha sido reiniciada correctamente. Ahora puedes continuar usando el bot normalmente. Usa !start para comenzar de nuevo."
          );
          return;
        case "pdf":
        case "url":
        case "link":
          ...
          if (session.lastPdfUrl) {
            await bot.sendMessage(
              from,
              `📊 *Aquí está el enlace a tu PDF de análisis:*\n\n${session.lastPdfUrl}`
            );
          } else {
            await bot.sendMessage(
              from,
              "No tienes ningún PDF generado recientemente. Envía tu CV para generar un análisis."
            );
          }
          return;
        case "promo":
          ...
          if (!code) {
            await bot.sendMessage(
              from,
              "Por favor, proporciona un código promocional. Usa: !promo TU_CODIGO"
            );
            return;
          }
    ...
    if (
      ...
      (text.toLowerCase().includes("pdf") ||
        ...
        text.toLowerCase().includes("analisis"))
    ) {
      // Enviar la URL del último PDF generado
      if (session.lastPdfUrl) {
        await bot.sendMessage(
    ...
    if (text.toLowerCase().startsWith("!promo ")) {
      ...
      if (!codeData) {
        await bot.sendMessage(
          from,
          "❌ El código promocional no es válido, ya ha sido usado o ha expirado."
        );
        return;
      }
      ...
      if (redeemed) {
        await bot.sendMessage(
          ...
          `✅ ¡Código promocional *${
            codeData.id
          }* activado con éxito! Ahora tienes acceso ilimitado.\nOrigen: ${
            codeData.source
          } (${codeData.description || ""})`
        );
        logger.info(
          `User ${from} successfully redeemed promo code ${codeData.id} from source ${codeData.source}`
        );
      } else {
        await bot.sendMessage(
          from,
          "⚠️ Hubo un problema al intentar canjear el código. Puede que alguien más lo haya usado justo ahora. Intenta de nuevo o contacta soporte."
        );
      }
      return;
    }
    ...
    switch (session.state) {
      case "initial":
        ...
        await sessionService.updateSessionState(
          from,
          sessionService.SessionState.MENU_SELECTION
        );
        break;
      case "terms_acceptance":
        ...
        if (
          ...
          await bot.sendMessage(
            from,
            "Para utilizar nuestros servicios es necesario aceptar los términos y condiciones. Sin esta aceptación, no podemos continuar."
          );
          await handleTermsAndConditions(from);
        } else {
          ...
          await bot.sendMessage(
            from,
            'Por favor, responde "Sí" si aceptas los términos y condiciones o "No" si los rechazas.'
          );
          await handleTermsAndConditions(from);
        }
        break;
      case sessionService.SessionState.MENU_SELECTION:
        ...
        if (
          text.toLowerCase().includes("revisar") ||
          text.toLowerCase().includes("cv")
        ) {
          await handleMenuSelection(from, "review_cv");
        } else if (
          text.toLowerCase().includes("simular") ||
          text.toLowerCase().includes("entrevista")
        ) {
      ...
      case sessionService.SessionState.WAITING_INTERVIEW_CONFIRMATION:
        ...
        if (
          ...
          text.toLowerCase().includes("comenzar")
        ) {
          ...
          await bot.sendMessage(
            from,
            "Entrevista cancelada. Si deseas volver a intentarlo, envía !start para comenzar de nuevo."
          );
          await sessionService.updateSessionState(
            from,
            sessionService.SessionState.INITIAL
          );
        } else {
          await bot.sendMessage(
            from,
            'Por favor, responde "sí" si estás listo para comenzar la entrevista o "no" para cancelar.'
          );
        }
        break;
      case sessionService.SessionState.POST_CV_OPTIONS:
        ...
        if (
          ...
          text.toLowerCase().includes("revisar") ||
          text.toLowerCase().includes("otro cv") ||
          ...
          if (shouldPay) {
            ...
            await bot.sendMessage(
              from,
              "Por favor, envía el nuevo CV que deseas analizar."
            );
      ...
      case sessionService.SessionState.WAITING_FOR_POSITION_BEFORE_CV:
        ...
        await bot.sendMessage(
          from,
          `Gracias por indicar el puesto de ${text}. Ahora, por favor envía tu CV en formato **PDF** como documento para analizarlo en relación con este puesto.`
        );
        await sessionService.updateSessionState(from, "waiting_for_cv");
        break;
      ...
      case "waiting_for_position_before_interview":
        ...
        await sessionService.saveJobPosition(from, text);
        logger.info(
          `Job position saved before interview for user ${from}: ${text}`
        );
      ...
      case sessionService.SessionState.POSITION_RECEIVED:
        ...
        if (text.toLowerCase() === "sí" || text.toLowerCase() === "si") {
          ...
          await bot.sendMessage(
            from,
            'Para comenzar la entrevista, responde "sí". Si deseas reiniciar el proceso, envía !reset.'
          );
        }
        break;
      case sessionService.SessionState.ANSWER_RECEIVED:
        ...
        if (text.toLowerCase() === "sí" || text.toLowerCase() === "si") {
          ...
          await bot.sendMessage(
            from,
            'Para continuar con la siguiente pregunta, responde "sí". Si deseas reiniciar el proceso, envía !reset.'
          );
        }
        break;
      case sessionService.SessionState.QUESTION_ASKED:
        ...
        await bot.sendMessage(
          from,
          "🎤 *¡Prefiero escucharte!* Por favor, responde a la pregunta con un mensaje de *audio* o *video* para que pueda evaluar mejor tu respuesta. ¡Esto hará el análisis mucho más completo! 😊"
        );
        break;
      case sessionService.SessionState.INTERVIEW_COMPLETED:
        ...
        logger.info(
          `User ${from} sent a message after interview completion. Auto-resetting session.`
        );
        await sessionService.resetSession(from);
        await handleStart(from);
        break;
      ...
      case "confirming_payment":
        if (
          text.toLowerCase().includes("pag") ||
          text.toLowerCase().includes("ya pag")
        ) {
          ...
          await bot.sendMessage(
            from,
            "Por favor, confirma si has realizado el pago o si deseas volver a la selección de paquetes."
          );
        }
        break;
      case "advisor_service_selection":
        if (
          text.toLowerCase().includes("acept") ||
          text.toLowerCase() === "si" ||
          text.toLowerCase() === "sí"
        ) {
          ...
          text.toLowerCase().includes("regres") ||
          text.toLowerCase().includes("volver") ||
          text.toLowerCase().includes("menu") ||
          text.toLowerCase().includes("cancel")
        ) {
          ...
          await bot.sendMessage(
            from,
            'Por favor, responde "ACEPTAR" si deseas continuar con la asesoría o "REGRESAR" para volver al menú principal.'
          );
        }
        break;
      case "selecting_advisor_type":
        if (
          ...
          await bot.sendMessage(
            from,
            'Por favor, indica si deseas asesoría para "Revisar CV" o "Simular Entrevista", o escribe "regresar" para volver al menú principal.'
          );
        }
        break;
      case "confirming_advisor_payment":
        if (
          ...
          await bot.sendMessage(
            from,
            "Por favor, confirma si has realizado el pago o si deseas volver a la información de asesoría."
          );
        }
        break;
      default:
        await bot.sendMessage(
          from,
          "Por favor, envía tu CV como documento para que pueda analizarlo."
        );
    }
...
¿Estás listo/a para comenzar la entrevista? Responde "sí" para continuar.
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al procesar tu respuesta. Por favor, intenta nuevamente."
    );
    throw error;
  }
};
...
const handleImage = async (from, image) => {
  try {
    logger.info(
      `Received image from user ${from}: ${JSON.stringify(image, null, 2)}`
    );
    ...
    if (session.state === "waiting_payment_screenshot") {
      ...
      logger.info(
        `Image received but user ${from} is not in payment verification flow. Current state: ${session.state}`
      );

      // Si estamos en estado de confirmación de pago, asumimos que la imagen es un comprobante
      if (session.state === "confirming_payment") {
        logger.info(
          `User ${from} is in confirming_payment state, treating image as payment screenshot`
        );
        ...
        logger.info(
          `User ${from} is in confirming_advisor_payment state, treating image as advisor payment screenshot`
        );
        // Actualizar el estado a waiting_advisor_payment_screenshot
        await sessionService.updateSessionState(
          from,
          "waiting_advisor_payment_screenshot"
        );
        ...
        await bot.sendMessage(
          from,
          "Gracias por la imagen. Por favor, envía !start si deseas comenzar a usar el bot o !help para obtener ayuda."
        );
      }
    }
  } catch (error) {
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al procesar tu imagen. Por favor, intenta nuevamente."
    );
  }
};
...
const handleAudio = async (from, audio) => {
  ...
  try {
    ...
    if (session.state !== sessionService.SessionState.QUESTION_ASKED) {
      logger.info(
        `[handleAudio] Estado incorrecto para recibir audio: ${session.state}`
      );
      await bot.sendMessage(
        from,
        "Por favor, espera a que te haga una pregunta antes de enviar una respuesta de audio."
      );
      return;
    }
    ...
    try {
      ...
      logger.info(
        `[handleAudio] Audio descargado (${audioBuffer.length} bytes)`
      );

      // Subida a R2
      try {
        const tempDir = os.tmpdir();
        const tempFilename = `interview_${from}_q${
        ...
        logger.error(
          `[handleAudio] Error subiendo audio a R2: ${uploadError.message}`
        );
      }
      ...
      try {
        ...
        transcription = await openaiUtil.transcribeAudio(audioBuffer, {
          language: "es",
          prompt: "Respuesta a pregunta de entrevista.",
        });
        logger.info(
          `[handleAudio] Transcripción: ${
            transcription ? transcription.substring(0, 50) : "No disponible"
          }`
        );
        if (transcription) {
          logger.info("[handleAudio] Analizando respuesta...");
          analysis = await openaiUtil.analyzeInterviewResponse(
            transcription,
        ...
        errorOccurred = true;
        logger.error(
          `[handleAudio] Error en transcripción/análisis: ${transcriptError.message}`
        );
      }
      ...
      if (errorOccurred || !analysis) {
        ...
        analysis =
          interviewService.generateMockInterviewAnalysis(currentQuestion);
      ...
      try {
        const updatedSessionData = await sessionService.getOrCreateSession(
          from
        );
        await interviewService.saveInterviewToFirestore(
          from,
          updatedSessionData
        );
        logger.info(
          `[handleAudio] Progreso guardado en Firestore para ${from}`
        );
      } catch (firestoreError) {
        logger.error(
          `[handleAudio] Error guardando en Firestore: ${firestoreError.message}`
        );
      }

      const feedbackMessage = formatInterviewFeedback(
        analysis,
        currentQuestion
      );
      ...
      if (
        finalSessionCheck.currentQuestion >= 3 ||
        finalSessionCheck.state ===
          sessionService.SessionState.INTERVIEW_COMPLETED
      ) {
        await sessionService.updateSessionState(
          from,
          sessionService.SessionState.INTERVIEW_COMPLETED
        ...
        try {
          const finalSessionData = await sessionService.getOrCreateSession(
            from
          );
          await interviewService.saveInterviewToFirestore(
            from,
          ...
          logger.info(
            `[handleAudio] Entrevista finalizada y guardada en Firestore para ${from}`
          );
        } catch (firestoreError) {
          logger.error(
            `[handleAudio] Error guardando estado final en Firestore: ${firestoreError.message}`
          );
        }
      ...
      logger.error(
        `[handleAudio] Error procesando audio: ${processingError.message}`
      );
      await bot.sendMessage(
        from,
        "😓 Lo siento, hubo un error al procesar tu respuesta. ¿Podrías intentar nuevamente?"
      );
    } finally {
      if (tempFilePath) {
        fs.unlink(tempFilePath).catch((err) =>
          logger.warn(
            `[handleAudio] No se pudo eliminar archivo temporal ${tempFilePath}: ${err.message}`
          )
        );
      }
    }
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error general al procesar tu audio."
    );
  }
};
...
const handleVideo = async (from, video) => {
  ...
  try {
    ...
    if (session.state !== sessionService.SessionState.QUESTION_ASKED) {
      logger.info(
        `[handleVideo] Estado incorrecto para recibir video: ${session.state}`
      );
      return;
    }
    ...
    try {
      ...
      logger.info(
        `[handleVideo] Video descargado (${videoBuffer.length} bytes)`
      );
      ...
      try {
        ...
        const tempVideoFilename = `interview_${from}_q${
          session.currentQuestion + 1
        }_${Date.now()}.mp4`;
        tempVideoPath = path.join(os.tmpdir(), tempVideoFilename);
        await fs.writeFile(tempVideoPath, videoBuffer);
        logger.info(
          `[handleVideo] Archivo temporal de video creado: ${tempVideoPath}`
        );
        ...
        logger.error(
          `[handleVideo] Error subiendo video a R2: ${uploadError.message}`
        );
      }
      ...
      try {
        ...
        errorOccurred = true;
        logger.error(
          `[handleVideo] Error en transcripción/análisis: ${transcriptError.message}`
        );
      }
      ...
      try {
        ...
        logger.info(
          `[handleVideo] Progreso guardado en Firestore para ${from}`
        );
      } catch (firestoreError) {
        logger.error(
          `[handleVideo] Error guardando en Firestore: ${firestoreError.message}`
        );
      }
      ...
      if (
        ...
        try {
          ...
          logger.info(
            `[handleVideo] Entrevista finalizada y guardada en Firestore para ${from}`
          );
        } catch (firestoreError) {
          logger.error(
            `[handleVideo] Error guardando estado final en Firestore: ${firestoreError.message}`
          );
        }
        await showPostInterviewMenu(from);
      } else {
      ...
      logger.error(
        `[handleVideo] Error procesando video: ${processingError.message}`
      );
      await bot.sendMessage(
        from,
        "😓 Lo siento, hubo un error al procesar tu respuesta en video. ¿Podrías intentar nuevamente?"
      );
    } finally {
      if (tempFilePath) {
        fs.unlink(tempFilePath).catch((err) =>
          logger.warn(
            `[handleVideo] No se pudo eliminar archivo temporal ${tempFilePath}: ${err.message}`
          )
        );
      }
    }
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error general al procesar tu video."
    );
  }
};
// ...existing code...
...
const handleSimulatedAnswer = async (from, session) => {
  try {
    ...
    if (
      updatedSession.state === sessionService.SessionState.INTERVIEW_COMPLETED
    ) {
...
Si necesitas ayuda adicional, escribe !help o contacta a nuestro equipo de soporte.`;
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al mostrar la ayuda. Por favor, intenta nuevamente más tarde."
    );
  }
};
...
const handleInterview = async (from) => {
  try {
    ...
    try {
      ...
      await sessionService.updateSessionState(
        from,
        sessionService.SessionState.WAITING_INTERVIEW_CONFIRMATION
      );
      logger.info(`Asked user ${from} if ready to start interview`);
    } catch (buttonError) {
      logger.warn(
        `Failed to send button message, using text message instead: ${buttonError.message}`
      );
      await bot.sendMessage(
        from,
        `Vamos a comenzar una simulación de entrevista para el puesto de ${jobPosition}. Te haré 4 preguntas y deberás responder con mensajes de audio o video.`
      ...
      await bot.sendMessage(
        from,
        '¿Estás listo para comenzar? Responde "sí" para iniciar la entrevista o "no" para cancelar.'
      );
      await sessionService.updateSessionState(
        from,
        sessionService.SessionState.WAITING_INTERVIEW_CONFIRMATION
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al preparar la entrevista. Por favor, intenta nuevamente con !start."
    );
  }
};
  ...
  try {
    ...
    try {
      ...
      if (openaiUtil.generateInterviewQuestion) {
        questionData = await openaiUtil.generateInterviewQuestion(
          jobPosition,
          questionPrompt
        );
      } else {
...
`;
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al iniciar la entrevista. Por favor, intenta nuevamente con !start."
    );
  }
};
    ...
    try {
      ...
      if (openaiUtil.generateInterviewQuestion) {
        ...
        if (session.questions && session.questions.length > 0) {
          ...
          while (
            previousQuestions.includes(questionData.question) &&
            attempts < 3
          ) {
            logger.info(
              `Pregunta repetida detectada, generando nueva pregunta (intento ${
                attempts + 1
              })`
            );
            questionData = await openaiUtil.generateInterviewQuestion(
              questionType,
              questionPrompt + " (diferente a las preguntas anteriores)"
            );
            attempts++;
          }
        }
...
`;
    ...
    await bot.sendMessage(from, questionMessage);
    logger.info(
      `Sent question ${nextQuestionNumber + 1} to user ${from}: "${
        questionData.question
      }"`
    );
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al generar la siguiente pregunta. Por favor, intenta nuevamente con !interview."
    );
  }
};
...
const handlePackageSelection = async (from, text) => {
  try {
    ...
    if (
      text.toLowerCase().includes("4") ||
      text.toLowerCase().includes("1 revisión") ||
      text.toLowerCase().includes("1 revision")
    ) {
      ...
      text.toLowerCase().includes("3 revisiones")
    ) {
      ...
      text.toLowerCase().includes("6 revisiones")
    ) {
      packageName = "6 Revisiones";
      packagePrice = "S/10";
      packageReviews = "6";
    } else if (
      text.toLowerCase().includes("15") ||
      text.toLowerCase().includes("10 revisiones")
    ) {
    ...
    try {
      ...
      logger.warn(
        `Failed to send payment confirmation buttons: ${buttonError.message}`
      );
      await bot.sendMessage(
        from,
        'Después de realizar el pago, responde con "pagado". Si quieres cambiar tu paquete, responde con "volver".'
      );
      await sessionService.updateSessionState(from, "confirming_payment");
    }
  } catch (error) {
...
const handlePaymentConfirmation = async (from) => {
  try {
    ...
    await bot.sendMessage(
      from,
      `✅ *Por favor, envía una captura de pantalla de tu pago de ${packagePrice}*\n\nNecesito verificar:\n• El Nro de operacion"\n• La fecha y hora`
    );
    ...
    await bot.sendMessage(
      from,
      "Ocurrió un error al procesar tu confirmación. Por favor, contacta con nuestro soporte."
    );
  }
};
...
const verifyPaymentScreenshot = async (from, image) => {
  try {
    ...
    logger.info(
      `Received payment screenshot from ${from} for ${packageReviews} reviews`
    );
    ...
    try {
      ...
      await bot.sendMessage(
        from,
        "No pudimos acceder a tu imagen. Por favor, intenta enviarla nuevamente."
      );
      return;
    }
    ...
    try {
      ...
      logger.info(
        `Payment image downloaded, size: ${imageBuffer.length} bytes`
      );
    } catch (downloadError) {
      logger.error(`Error downloading image: ${downloadError.message}`);
      await bot.sendMessage(
        from,
        "Hubo un problema al descargar tu imagen. Por favor, intenta enviarla nuevamente."
      );
      return;
    }
...
- reason: razón por la que es válido o inválido`;
      ...
      const imageAnalysis = await openaiUtil.analyzeImage(
        imageBase64,
        systemPrompt,
        userPrompt
      );
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          logger.info(
            `Parsed analysis result: ${JSON.stringify(analysisResult)}`
          );
        } else {
          // Si no encuentra JSON, intentar extraer la validez de la respuesta
          logger.warn(
            "No JSON found in OpenAI response, using text analysis fallback"
          );
          isValidPayment =
            imageAnalysis.toLowerCase().includes("válido") ||
      ...
      if (!analysisResult.isValid) {
        logger.info(
          "Payment marked as invalid by OpenAI, checking for critical elements"
        );
        ...
        const isYapeOrPlin =
          imageAnalysis.toLowerCase().includes("yape") ||
        ...
        if (
          (hasCorrectName ||
            imageAnalysis.toLowerCase().includes("francesco")) &&
          (hasCorrectAmount ||
            imageAnalysis.toLowerCase().includes(packagePrice))
        ) {
          logger.info(
            "Critical elements found (name and amount), overriding OpenAI result to VALID"
          );
          analysisResult.isValid = true;
          analysisResult.reason =
            "Pago verificado: contiene el nombre y monto correctos";
        }
      }
      ...
      if (isValidPayment) {
        ...
        try {
          ...
          logger.warn(
            `Failed to send post-payment buttons: ${buttonError.message}`
          );
          // Si no se pueden enviar los botones, enviar mensaje normal
          await bot.sendMessage(
            from,
            "Para usar tus créditos, simplemente envía el CV que deseas analizar o escribe !start para ir al menú principal."
          );
          await sessionService.updateSessionState(from, "waiting_for_cv");
        }
      } else {
        ...
        logger.warn(
          `Invalid payment image from user ${from}: ${analysisResult.reason}`
        );

        // Informar al usuario por qué el pago fue rechazado
        let rejectionReason =
          "no pudimos verificar que cumpla con los requisitos";
        ...
        if (analysisResult.reason) {
          ...
          if (
            analysisResult.amount &&
            analysisResult.amount !== packagePrice.replace("S/", "")
          ) {
            ...
            rejectionReason =
              "el destinatario no parece ser Francesco Lucchesi";
          } else {
      ...
      await bot.sendMessage(
        from,
        "❌ Lo sentimos, tuvimos un problema técnico al verificar tu pago. Por favor, intenta nuevamente en unos minutos o contacta a soporte si el problema persiste."
      );
    }
  } catch (error) {
    ...
    await bot.sendMessage(
      from,
      "Ocurrió un error al verificar tu pago. Por favor, contacta con nuestro soporte."
    );
  }
};
  ...
  try {
    ...
    switch (buttonId) {
      ...
      case "stop_interview":
        await bot.sendMessage(
          from,
          "Has detenido la entrevista. Si deseas volver a intentarlo, envía !start para comenzar de nuevo."
        );
        await sessionService.resetSession(from);
        await handleStart(from);
        break;
      ...
      case "accept_advisor":
        await bot.sendMessage(
          from,
          `Yapea o Plinea S/60 a este número:\n954600805\n\nEstá a nombre de "Francesco Lucchesi"`
        );
        ...
        try {
          ...
          await bot.sendMessage(
            from,
            'Después de realizar el pago, responde con "pagado". Si quieres volver, responde con "volver".'
          );
          await sessionService.updateSessionState(
            from,
            "confirming_advisor_payment"
...
`;
        ...
        try {
          ...
          await sessionService.updateSessionState(
            from,
            "advisor_service_selection"
          );
        } catch (buttonError) {
          logger.warn(
            `Failed to send advisor service buttons: ${buttonError.message}`
          );
      ...
      default:
        ...
        await bot.sendMessage(
          from,
          "No reconocí esa opción. Por favor, envía !start para comenzar de nuevo."
        );
    }
  } catch (error) {
    ...
    try {
      ...
      */
      ...
      try {
        ...
        logger.warn(
          `Failed to send advisor button: ${advisorButtonError.message}`
        );
      ...
      await bot.sendMessage(
        from,
        "Lo siento, hubo un error al enviar las opciones de post-análisis. Por favor, intenta nuevamente."
      );
    }
  } catch (error) {
...
const handleAdvisorService = async (from) => {
  try {
    ...
    try {
      ...
      logger.warn(
        `Failed to send advisor type buttons: ${buttonError.message}`
      );
      await bot.sendMessage(
        from,
        'Por favor, responde si deseas una asesoría para "Revisar CV" o "Simular Entrevista".'
      );
      await sessionService.updateSessionState(from, "selecting_advisor_type");
    }
  } catch (error) {
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al procesar tu solicitud. Por favor, intenta nuevamente con !start."
    );
  }
};
...
const handleAdvisorPaymentConfirmation = async (from) => {
  try {
    ...
    await bot.sendMessage(
      from,
      `✅ *Por favor, envía una captura de pantalla de tu pago de S/60*\n\nNecesito verificar:\n• El nro de operacion"\n• La fecha y hora sea reciente`
    );
    ...
    logger.error(
      `Error handling advisor payment confirmation: ${error.message}`
    );
    await bot.sendMessage(
      from,
      "Ocurrió un error al procesar tu confirmación. Por favor, contacta con nuestro soporte."
...
const verifyAdvisorPaymentScreenshot = async (from, image) => {
  try {
    ...
    try {
      imageBuffer = await fileProcessing.downloadFile(imageUrl);
      logger.info(
        `Advisor payment image downloaded, size: ${imageBuffer.length} bytes`
      );
    } catch (downloadError) {
      logger.error(`Error downloading payment image: ${downloadError.message}`);
      await bot.sendMessage(
        from,
        "Hubo un error al procesar tu imagen. Por favor, intenta nuevamente."
      );
      return;
    }
...
- reason: razón por la que es válido o inválido (enfocándose en nombre y monto)`;
      ...
      try {
        ...
        if (jsonMatch) {
          analysisResult = JSON.parse(jsonMatch[0]);
          logger.info(
            `Parsed advisor analysis result: ${JSON.stringify(analysisResult)}`
          );
        } else {
          // Si no encuentra JSON, intentar extraer la validez de la respuesta
          logger.warn(
...
Si tienes alguna duda, no dudes en escribirnos.`
        ...
        await sessionService.updateSessionState(
          from,
          "advisor_payment_completed"
        );
      } else {
        // El pago no es válido
        logger.warn(
          `Invalid advisor payment image from user ${from}: ${analysisResult.reason}`
        );

        // Determinar la razón del rechazo
        let rejectionReason =
          analysisResult.reason || "no pudimos verificar claramente el pago";
      ...
      await bot.sendMessage(
        from,
        "Hubo un error al verificar tu pago. Por favor, intenta enviar la imagen nuevamente."
      );
    }
  } catch (error) {
    ...
    await bot.sendMessage(
      from,
      "Lo siento, hubo un error al procesar tu pago. Por favor, intenta nuevamente más tarde."
    );
  }
};
...
Al continuar, aceptas nuestros términos, nuestra política de privacidad.
    ...
    try {
      ...
      await bot.sendButtonMessage(from, termsMessage, termsButtons);
    } catch (buttonError) {
...
¿Cómo te gustaría que te ayude hoy?
    ...
    try {
      ...
      await bot.sendButtonMessage(from, welcomeMessage, menuButtons, "");
      ...
      logger.warn(
        `Failed to send button message, using text fallback: ${buttonError.message}`
      );
    ...
    await bot.sendMessage(
      from,
      "😓 Lo siento, ha ocurrido un error. Por favor, intenta nuevamente enviando *!start*."
    );
  }
};
